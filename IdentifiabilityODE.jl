...
function DifferentiateOnce(diff_poly, var_list)
    result = 0
    rng = parent(diff_poly)
    for diff_v in vars(diff_poly)
       v,h = GetOrderVar(diff_v,parent(var_list[1]))
       if v in var_list
          result = result + derivative(diff_poly, str_to_var(MakeDerivative(v, h),parent(diff_poly))) * str_to_var(MakeDerivative(v, h + 1),parent(diff_poly))
       end
    end
    return(result)
end
...
function eval_frac_num(frac, vars, vals)
  if iszero(frac)
    ...
    return(parent(fr[1])(Nemo.lc(evaluate(fr[1],vars,vals))//Nemo.lc(evaluate(fr[2],vars,vals))))
  end
end

function eval_frac_fmpq(frac, vars, vals)
  if iszero(frac)
    return 0
  else
    fr = unpack_fraction(frac)
    return(fmpq(Nemo.lc(evaluate(fr[1],vars,vals))//Nemo.lc(evaluate(fr[2],vars,vals))))
  end
end
...
function get_fmpq_mpoly(frac)
  fr = unpack_fraction(frac)
  return parent(fr[1])(Nemo.lc(fr[1])//Nemo.lc(fr[2]))
end

function SamplePoint2(bound, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
  local s, theta_hat, u_hat, x_hat, y_hat, all_subs, to_compute
  s = length(all_params)
  ...
  while true
     ...
     for i in 0:(s+1)
        for j in 1:length(y_vars)
           vl = eval_frac_fmpq(Y_eq[(j-1)*(s+2)+i+1][2],all_subs[1],all_subs[2])
        ...
        for j in 1:length(x_vars)
           vl = eval_frac_fmpq(X_eq[(j-1)*(s+2)+i+1][2],all_subs[1],all_subs[2])
...
function SamplePoint3(bound, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
  local s, theta_hat, u_hat, x_hat, y_hat, all_subs, to_compute
  s = length(all_params)
  y_hat_vars = Array{fmpq_mpoly}(undef,0)
  y_hat_vals = Array{fmpq}(undef,0)
...
function SamplePoint(bound, x_vars, y_vars, u_vars, mu, X_eq, Y_eq, Q)
  ...
  while true
     theta_hat = [fmpq(rnd) for rnd in rand(0:bound, length(all_params))]
...
function JacobiMatrix3(pol_arr, vrs, vals)
...
function JacobiMatrix2(pol_arr, vals)
  vrs = gens(parent(pol_arr[1]))
  m = Nemo.MatrixSpace(Nemo.QQ,length(pol_arr),length(vrs))()
  for i in 1:length(pol_arr)
    for j in 1:length(vrs)
      m[i,j] = evaluate(derivative(pol_arr[i],vrs[j]),vals)
...
function JacobiMatrix(pol_arr, vars, vals)
  m = Nemo.MatrixSpace(Nemo.QQ,length(pol_arr),length(vars))()
  for i in 1:length(pol_arr)
    for j in 1:length(vars)
      m[i,j] = fmpq(Nemo.lc(evaluate(derivative(pol_arr[i],vars[j]),vars,vals)))
    end
  end
  return(m)
end
...
function GetVars2(diff_poly, var_list,shft,s)
...
function CompareDiffVar2(dvl, dvr, non_jet_ring, shft, s)
  local vl, vr, hl, hr;
...
# 2.Truncate
  ...
  # (c, d) ---------------
  #sample = SamplePoint2(D1, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
  sample = SamplePoint3(D1, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
  all_subs = sample[4]
  u_hat = sample[2]
  y_hat = sample[1]
  ...
  for i in 1:m
    ...
    #    end
    #  end
      for v in GetVars2(Y[i][j], x_vars, n+m+u, s)
        if !(v in x_theta_vars)
          to_add = false
        end
      end
  ...
  if length(theta_l) > 0
    ...
    # (b, c) ---------
    #sample = SamplePoint2(D2, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
    sample = SamplePoint3(D2, x_vars, y_vars, u_variables, all_params, X_eq, Y_eq, Q)
    y_hat = sample[1]
    u_hat = sample[2]
    theta_hat = sample[3]
 ...
 #   end
    for poly in Et_hat
      Et_x_vars = union(Et_x_vars, GetVars2(poly, x_vars, n+m+u, s))
    end
    Q_hat = evaluate(Q, u_hat[1], u_hat[2])
    #vrs_sorted = vcat(sort([e for e in Et_x_vars], lt=(x,y)->CompareDiffVar(x,y,non_jet_ring)), z_aux, sort(not_int_cond_params,rev=true))
    vrs_sorted = vcat(sort([e for e in Et_x_vars], lt=(x,y)->CompareDiffVar2(x,y,non_jet_ring,n+m+u,s)), z_aux, sort(not_int_cond_params,rev=true))
    # 4. Determine.
    println(Time(now()))
